...
We start by loading the data and packages (which have already been installed).
...
There are a lot of decimal places in our proportion tables. We can use the `round()` function to simplify. Note the `,3` option tied to the parentheses closing the `round()` function. That 3 asserts we want the proportions rounded to 3 decimal places.
...
The last figure we will make is a historical trend line showing racial differences in responses to the `conmedic` variable. We start by finding, for each year and race combination, the proportion with each response to the `conmedic` question. We'll use the `racehisp` variable (created from the `race` and `anyhispanic` variables) here.
...
Next we reshape the data so there is one row for each year/race/confidence combination and all the proportions are in one column. We will also fix the order of the `conmedic` categories (with the `levels` option) and give them real names (with the `labels` option).
