...
There are a lot of decimal places there. We can use the `round()` function to simplify. Note the `,3` option tied to the parentheses closing the `round()` function. That 3 asserts we want the proportions rounded to 3 decimal places.
...
Let's see how the age distribution in internet political usage usage varies by sex. Once you are working with at least three variables, it is more efficient to use the `group_by` and `summarize` functions to calculate measures for specific combinations of categorical levels.

```{r}
political_sex_table <- gss %>%
     group_by(age_category, sex) %>%
     filter(age_category!="NA") %>% # Use filter to restrict observations
     summarize(political_internet = mean(polinter=="Have done it in the past year", na.rm=TRUE))
...
conmedic_plot + geom_point() + geom_line() + facet_grid(.~race) +
     labs(x = "Year",
          y = "Proportion",
          title = "Racial Differences in Confidence in Medicine",
          subtitle = "General Social Survey, 1996-2018",
          color = "") +
     theme(legend.position = "bottom")
```
