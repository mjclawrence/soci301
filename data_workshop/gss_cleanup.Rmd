...
```{r}
gss <- gss %>%
  mutate(anyhispanic = ifelse(hispanic == 0 | hispanic >= 98, NA,
                              ifelse(hispanic == 1, "No", "Yes")))
```

```{r}
gss <- gss %>%
  mutate(racehisp = ifelse(anyhispanic == "Yes", "Hispanic", race),
...
```{r}
gss <- gss %>%
  select(-id_, -ballot, -(disabld1:disabld5), -educ, -hispanic)
```
